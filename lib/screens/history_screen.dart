        ...
        : _txns.isEmpty
          ? ListView(children: [
              ...
              Center(child: Column(children: [
                ...
                const SizedBox(height: 16),
                Text('No transactions yet', style: GoogleFonts.spaceGrotesk(
...
class _TxnTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final currency = txn['currency'] as String? ?? 'VND';
    final category = txn['category'] as String? ?? 'Other';
    final wallet   = txn['wallet'] as String? ?? '';
    final address  = txn['address'] as String?;
    final note     = txn['note'] as String?;
    final amtFmt   = NumberFormat('#,##0', 'vi_VN').format(amount);

    DateTime? dt;
    try { dt = DateTime.parse(txn['date_time']).toLocal(); } catch (_) {}
      ...
      child: Row(children: [
        Container(width: 44, height: 44,
          ...
          child: Icon(isExpense ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded,
            color: isExpense ? kExpense : kIncome, size: 20)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(category, style: GoogleFonts.dmSans(
            fontWeight: FontWeight.w700, fontSize: 14.5, color: kText)),
          if (address != null && address.isNotEmpty)
            Text(address, style: GoogleFonts.dmSans(fontSize: 12.5, color: kSubtext)),
          if (note != null && note.isNotEmpty)
            Text(note, style: GoogleFonts.dmSans(fontSize: 12, color: kSubtext.withOpacity(0.7)),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 2),
          Row(children: [
            Icon(Icons.account_balance_wallet_outlined, size: 12, color: kSubtext),
            const SizedBox(width: 3),
            Text(wallet, style: GoogleFonts.dmSans(fontSize: 11.5, color: kSubtext)),
            if (dt != null) ...[
              const SizedBox(width: 8),
              Text('• ${DateFormat('dd/MM HH:mm').format(dt)}',
                style: GoogleFonts.dmSans(fontSize: 11.5, color: kSubtext)),
            ],
          ]),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text('${isExpense ? '-' : '+'}$amtFmt',
            style: GoogleFonts.spaceGrotesk(
              fontSize: 15, fontWeight: FontWeight.w700,
              color: isExpense ? kExpense : kIncome)),
          Text(currency, style: GoogleFonts.dmSans(fontSize: 11.5, color: kSubtext)),
        ]),
      ]),
    );
  }
