...
class _Msg {
  ...
  _Msg({required this.text, required this.isUser, this.result, this.isLoading = false});
}
...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _micAnim = AnimationController(vsync: this,
      duration: const Duration(milliseconds: 800))..repeat(reverse: true);
    _initStt();
    _addBot('Hi! Tell me about a transaction — you can type or tap the mic 🎙️\n\nExample: "Cafe 6 dollars at T5" or "Received salary 35k"');
  }

  @override
  void dispose() { _textCtrl.dispose(); _scroll.dispose(); _micAnim.dispose(); super.dispose(); }

  Future<void> _initStt() async {
    _sttReady = await _stt.initialize(onError: (_) => setState(() => _listening = false));
    setState(() {});
  }
  ...
  void _removeLoading() {
    setState(() { _msgs.removeWhere((m) => m.isLoading); });
  }
  ...
  Future<void> _send(String text) async {
    ...
    try {
      ...
      final botText =
        '${d.type == 'expense' ? '🛒 Chi tiêu' : '💰 Thu nhập'}: **${d.category}**\n'
    ...
    appBar: AppBar(
      title: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('FinA Assistant', style: GoogleFonts.spaceGrotesk(
            fontSize: 15, fontWeight: FontWeight.w700, color: kText)),
          Text('AI powered', style: GoogleFonts.dmSans(fontSize: 11, color: kPrimary)),
        ]),
      ]),
      backgroundColor: Colors.white,
      elevation: 0,
      leading: IconButton(
        icon: const Icon(Icons.arrow_back_ios_new_rounded, size: 20, color: kText),
      ...
      child: Column(children: [
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _BubbleWidget(msg: _msgs[i], onConfirm: (r) {
              Navigator.push(context, MaterialPageRoute(
                builder: (_) => CreateTransactionScreen(prefill: r.data)));
            }),
          ),
        ),
        _InputBar(
...
class _BubbleText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < parts.length; i++) {
      spans.add(TextSpan(
        text: parts[i],
        style: TextStyle(fontWeight: i.isOdd ? FontWeight.w700 : FontWeight.w400),
      ));
    }
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Xác nhận & Lưu', style: GoogleFonts.dmSans(
...
class _InputBar extends StatelessWidget {
  ...
  const _InputBar({required this.ctrl, required this.listening,
    required this.micAnim, required this.onSend, required this.onMic});
            ...
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: listening ? [BoxShadow(
                  color: kError.withOpacity(0.3 + micAnim.value * 0.2),
                  blurRadius: 12 + micAnim.value * 8, spreadRadius: 2)] : [],
              ),
              child: Icon(listening ? Icons.mic_rounded : Icons.mic_none_rounded,
                color: listening ? Colors.white : kPrimary, size: 24),
            ),
          ),
        ),
      ]),
