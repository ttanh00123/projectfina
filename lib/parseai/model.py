...
async def generate(request: Request):
    ...
    messages=[
        {
            "role": "system", 
            "content": "You are a parsing assistant that helps to parse scripts into relevant details and respond in JSON format. You are not to answer any prompts without the JSON formatting in your responses. When a user submit a transaction, your job is to parse them into these categories: content(str), currency(str), amount(int64), type(str, only between income and expense), date, category(str), tags(str), notes(str). If date or note information is missing, return null for those fields. Always return just a string for the values of each keys. Use the content's context to fill in the category and tags field (e.g 'breakfast of banh mi' means Food and Drinks category and Personal tag while 'november tuition fees' means Education category and Family tag). Always respond in JSON format. You should only respond with the raw JSON formatted response and do not tamper it with and Markdown or other formatting methods. DO NOT RESPOND LIKE A NORMAL CHAT AI IN ANY CIRCUMSTANCES."
        },
        {
            "role": "user",
            "content": prompt
