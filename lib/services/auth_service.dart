...
Future<UserModel?> loginWithToken() async {
  ...
  if (response.statusCode == StatusCode.OK) {
    ...
    if (ok) {
      UserModel user = UserModel.fromJson(userData['data']['user']);
...
Future<UserModel?> loginWithPassword(
  ...
  if (response.statusCode == StatusCode.OK) {
      ...
      if (userData['result']) {
        ...
        Session.setSession(jwt: token, id: user.userId, mail: user.email);
...
Future updateFCMTokenIfNeeded(UserModel user) async {
  print('FMC Token= {}'.format(user.pushNotiToken ?? ""));
  if (user.pushNotiToken == null || user.pushNotiToken!.isEmpty) {
    print('FMC Token is empty');
    final fcmToken = await FirebaseMessaging.instance.getToken();
    updateFCMToken(user.userId, fcmToken);
  }
}
